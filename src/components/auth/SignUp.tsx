...
import { FormAction } from "@/types/auth";
...
const SignUp = ({ action }: SignUpProps) => {
  ...
  return (
    <Form action={formAction} className="max-w-md mx-auto bg-white rounded-lg shadow-md p-8 my-16">
      <h1 className="text-2xl font-bold text-center mb-2">Join MingleMart</h1>
      <p className="text-center text-sm text-rose-600 font-semibold mb-2">🔥 LIMITED TIME OFFER 🔥</p>
      <p className="text-center text-sm text-gray-600 mb-2">Sign up now and pay normal prices!</p>
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <input type="password" name="confirmPassword" id="confirmPassword" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-rose-500 focus:border-rose-500 sm:text-sm" />
        </div>
        ...
        {state?.message && state.message.length > 0 && <div className="text-center text-sm text-red-600">{state.message}</div>}
      </div>
    </Form>
  );
};
